defmodule App.Infrastructure.EntryPoint.ApiRest do

  @moduledoc """
  Access point to the rest exposed services
  """
  alias App.Domain.Model.Equipo
  alias App.Domain.UseCase.EquipoUseCase
  alias App.Utils.DataTypeUtils
  alias App.Infrastructure.EntryPoint.ErrorHandler
  require Logger
  use Plug.Router
  use Timex

  plug(CORSPlug,
    methods: ["GET", "POST", "PUT", "DELETE"],
    origin: [~r/.*/],
    headers: ["Content-Type", "Accept", "User-Agent"]
  )

  plug(Plug.Logger, log: :debug)
  plug(:match)
  plug OpentelemetryPlug.Propagation
  plug(Plug.Parsers, parsers: [:urlencoded, :json], json_decoder: Poison)
  plug(Plug.Telemetry, event_prefix: [:app, :plug])
  plug(:dispatch)

  forward(
    "/app/api/health",
    to: PlugCheckup,
    init_opts: PlugCheckup.Options.new(json_encoder: Jason, checks: App.Infrastructure.EntryPoint.HealthCheck.checks)
  )

  #Guardar equipo
  post "/app/api/crear-equipo/" do
    body = Plug.Conn.read_body(conn)
    {:ok, "", %Plug.Conn{params: params}} = body
    case guardar_equipo(params) do
      {:ok, equipo} -> equipo |> build_response(conn)
      {:error, error} -> %{status: 500, body: error} |> build_response(conn)
    end
  end

  defp guardar_equipo(equipo_params) do
    EquipoUseCase.crear_equipo(%Equipo{id: nil, nombre: equipo_params["nombre"], abreviatura: equipo_params["abreviatura"]})
  end

  #buscar equipo por id
  get "/app/api/buscar-equipo/:id" do
    id_str = conn.params["id"]
    id = String.to_integer(id_str)
    EquipoUseCase.buscar_equipo_por_id(id) |> build_response(conn)
  end

  def build_response(%{status: status, body: body}, conn) do
    conn
    |> put_resp_content_type("application/json")
    |> send_resp(status, Poison.encode!(body))
  end

  def build_response(response, conn), do: build_response(%{status: 200, body: response}, conn)

  match _ do
    conn
    |> handle_not_found(Logger.level())
  end

  defp handle_error(error, conn) do
    error
    |> ErrorHandler.build_error_response()
    |> build_response(conn)
  end

  defp handle_bad_request(error, conn) do
    error
    |> ErrorHandler.build_error_response()
    |> build_bad_request_response(conn)
  end

  defp build_bad_request_response(response, conn) do
    build_response(%{status: 400, body: response}, conn)
  end

  defp handle_not_found(conn, :debug) do
    %{request_path: path} = conn
    body = Poison.encode!(%{status: 404, path: path})
    send_resp(conn, 404, body)
  end

  defp handle_not_found(conn, _level) do
    send_resp(conn, 404, "")
  end
end
